// Register-only kernel for 1s+3p shell quartets (L_sum=3).
// Included multiple times with different S_POS and KERNEL_NAME.
//
// Expected macros:
//   S_POS       - position of the s shell (0-3)
//   KERNEL_NAME
//
// No #pragma once.
    ...
    int shell_buf_size, float prim_screen_tol, int n_fock_copies)
{
    ...
    const int l[4] = {_1S3P_IS_P(0), _1S3P_IS_P(1), _1S3P_IS_P(2),
                      _1S3P_IS_P(3)};
    const int dim_c[4] = {_1S3P_IS_P(0) ? 3 : 1, _1S3P_IS_P(1) ? 3 : 1,
                          _1S3P_IS_P(2) ? 3 : 1, _1S3P_IS_P(3) ? 3 : 1};
    ...
    SIMPLE_DEVICE_FOR(task_id, n_tasks)
    {
            ...
            shell_screen_tol)
        {
            ...
            float eri_cart[27];  // max 3*3*3 for 1s3p
