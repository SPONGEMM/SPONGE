    ...
    int shell_buf_size, float prim_screen_tol)
{
    SIMPLE_DEVICE_FOR(task_id, n_tasks)
    {
        ...
        const float coul_screen =
            shell_bound * fmaxf(pair_density_coul[ij_pair], pair_density_coul[kl_pair]);
        ...
        if (fmaxf(coul_screen, fmaxf(exx_screen_a, exx_screen_b)) >= shell_screen_tol)
        {
        // ---- Shell data ----
        ...
        const float AB[3] = {RC[0][0]-RC[1][0], RC[0][1]-RC[1][1], RC[0][2]-RC[1][2]};
        ...
        const float ai = env[p_exp_off[0]+ip], ci = env[p_cof_off[0]+ip];
        for (int jp = 0; jp < np[1]; jp++) {
            ...
            const float PA[3]={Px-RC[0][0],Py-RC[0][1],Pz-RC[0][2]};
        for (int kp = 0; kp < np[2]; kp++) {
        const float ak = env[p_exp_off[2]+kp], ck = env[p_cof_off[2]+kp];
        for (int lp = 0; lp < np[3]; lp++) {
            ...
            const float pref = 2.0f*PI_25/(p_val*q_val*sqrtf(p_val+q_val));
            const float n_abcd = n_ab*ck*env[p_cof_off[3]+lp]*kcd*pref;
            if (fabsf(n_abcd) < prim_screen_tol) continue;
            const float Qx=(ak*RC[2][0]+al*RC[3][0])*inv_q;
            const float Qy=(ak*RC[2][1]+al*RC[3][1])*inv_q;
            const float Qz=(ak*RC[2][2]+al*RC[3][2])*inv_q;
            const float QC_v[3]={Qx-RC[2][0],Qy-RC[2][1],Qz-RC[2][2]};
            const float PQ[3]={Px-Qx,Py-Qy,Pz-Qz};
            const float rho = p_val*q_val/(p_val+q_val);
            const float T = rho*(PQ[0]*PQ[0]+PQ[1]*PQ[1]+PQ[2]*PQ[2]);
            ...
            for (int ir = 0; ir < ERI_NRYS; ir++)
            {
                ...
                const float B10 = 0.5f/p_val * (1.0f - q_val*factor);
                ...
                for (int c1 = 0; c1 < dim_cart[1]; c1++) {
                    int ax1, ay1, az1;
                    QC_Get_Lxyz_Device(l[1], c1, ax1, ay1, az1);
                for (int c2 = 0; c2 < dim_cart[2]; c2++) {
                    int ax2, ay2, az2;
                    QC_Get_Lxyz_Device(l[2], c2, ax2, ay2, az2);
        ...
        if (is_spherical)
        {
            int cur_dim[4] = {dim_cart[0], dim_cart[1], dim_cart[2], dim_cart[3]};
            ...
            for (int si = 0; si < 4; si++)
            {
                ...
                if (nc == ns && l[si] == 0)
                {
                    float c2s = cart2sph_mat[ao_offsets_cart[sh[si]]*nao_sph + off[si]];
                    ...
                    for (int s = 0; s < ns; s++)
                        for (int inn = 0; inn < stride_si; inn++)
                        {
                            ...
                            for (int cc = 0; cc < nc; cc++)
                                sum += (double)cart2sph_mat[(oc+cc)*nao_sph+(os+s)] *
                                       (double)buf_a[o*nc*stride_si + cc*stride_si + inn];
                            buf_b[o*ns*stride_si + s*stride_si + inn] = (float)sum;
                        }
              ...
              for (int c1 = 0; c1 < dim_eff[1]; c1++)
                for (int c2 = 0; c2 < dim_eff[2]; c2++)
                  for (int c3 = 0; c3 < dim_eff[3]; c3++)
                      eri_out[idx++] *= norms[off[0]+c0]*norms[off[1]+c1]*
        ...
        const bool jk_same_braket = (tk.x == tk.z && tk.y == tk.w);
        {
            ...
            if (jk_same_bra && q > p) { idx += dim_eff[2]*dim_eff[3]; continue; }
            for (int c2 = 0; c2 < dim_eff[2]; c2++) {
            ...
            for (int c3 = 0; c3 < dim_eff[3]; c3++) {
                ...
                if (jk_same_braket)
                    if (QC_AO_Pair_Index(r,s) > QC_AO_Pair_Index(p,q)) continue;
                if (val == 0.0f) continue;
                if (!jk_same_bra && !jk_same_ket && !jk_same_braket)
                    QC_Accumulate_Fock_General_Quartet(p,q,r,s,val,nao,P_coul,P_exx_a,
                        P_exx_b,exx_scale_a,exx_scale_b,F_a_accum,F_b_accum);
                else
                    QC_Accumulate_Fock_Unique_Quartet(p,q,r,s,val,nao,P_coul,P_exx_a,
                        P_exx_b,exx_scale_a,exx_scale_b,F_a_accum,F_b_accum);
            }}}}
        }
        } // screening
