/*
Basically taken from github "tbtSVD", whose author is "wi-re", and Xia Yijie &
Yu Hao modified a little bit for SPONGE https://github.com/wi-re/tbtSVD commit
21e06aa MIT license A. McAdams, A. Selle, R. Tamstorf, J. Teran and E. Sifakis,
“Computing the Singular Value Decomposition of 3x3 matrices with minimal
branching and elementary floating point operations”, University of Wisconsin -
Madison technical report TR1690, May 2011
*/
...
// Iteration counts for Jacobi Eigenanlysis and reciprocal square root
// functions, influence precision
#define JACOBI_STEPS 12
#define RSQRT_STEPS 4
#define RSQRT1_STEPS 6

// Calculates the result of x / y. Required as the accurate square root function
// otherwise uses a reciprocal approximation when using optimizations on a GPU
// which can lead to slightly different results. If non exact matching results
// are acceptable a simple division can be used. Maps to __fdiv_rn(x,y)
// internally for CUDA.
__device__ __forceinline__ float fdiv(float x, float y)
{
#ifndef USE_GPU
    return x / y;
#else
...
// Calculates the reciprocal square root of x using a fast approximation. The
// number of newton iterations can be controlled using RSQRT_STEPS. A built-in
// rsqrt function or 1.f/sqrt(x) could be used, however doing this manually
// allows for exact matching results on CPU and GPU code.
...
// Calculates the square root of x using 1.f/rsqrt1(x) to give a square root
// with controllable and consistent precision on CPU and GPU code.
__device__ __forceinline__ float accurateSqrt(float x)
{
...
struct quaternion
{
    ...
    __device__ __forceinline__ float& operator[](int32_t arg)
    {
...
struct Mat3x3
{
    ...
    static __device__ __forceinline__ Mat3x3 fromPtr(float* ptr, int32_t i,
                                                     int32_t offset)
    {
        return Mat3x3{
    ...
    __device__ __forceinline__ Mat3x3(float a11 = 1.f, float a12 = 0.f,
                                      float a13 = 0.f, float a21 = 0.f,
                                      float a22 = 1.f, float a23 = 0.f,
                                      float a31 = 0.f, float a32 = 0.f,
                                      float a33 = 1.f)
        : m_00(a11),
          m_01(a12),
          m_02(a13),
    ...
    __device__ __forceinline__ Mat3x3(const quaternion& q)
    {
    ...
    __device__ __forceinline__ Mat3x3 transpose() const
    {
        return Mat3x3{m_00, m_10, m_20, m_01, m_11, m_21, m_02, m_12, m_22};
    }
    __device__ __forceinline__ Mat3x3 operator*(const float& o) const
    {
        return Mat3x3{
    ...
    __device__ __forceinline__ Mat3x3& operator*=(const float& o)
    {
        m_00 *= o;
        m_01 *= o;
        m_02 *= o;
    ...
    __device__ __forceinline__ Mat3x3 operator-(const Mat3x3& o) const
    {
...
// A simple symmetrix 3x3 Matrix class (contains no storage for (0, 1) (0, 2)
// and (1, 2)
struct Symmetric3x3
{
    float m_00 = 1.f;
    ...
    float m_20 = 0.f, m_21 = 0.f, m_22 = 1.f;
    __device__ __forceinline__ Symmetric3x3(float a11 = 1.f, float a21 = 0.f,
                                            float a22 = 1.f, float a31 = 0.f,
                                            float a32 = 0.f, float a33 = 1.f)
        : m_00(a11), m_10(a21), m_11(a22), m_20(a31), m_21(a32), m_22(a33)
    {
    }
    __device__ __forceinline__ Symmetric3x3(Mat3x3 o)
        : m_00(o.m_00),
          m_10(o.m_10),
          m_11(o.m_11),
          m_20(o.m_20),
...
// and z are 0)
__device__ __forceinline__ givens approximateGivensQuaternion(Symmetric3x3& A)
{
...
// updates the quaternion to contain the cumultative rotation
__device__ __forceinline__ void jacobiConjugation(const int32_t x,
                                                  const int32_t y,
                                                  const int32_t z,
                                                  Symmetric3x3& S,
                                                  quaternion& q)
...
// cumultative result used to reconstruct S
__device__ __forceinline__ quaternion jacobiEigenanlysis(Symmetric3x3 S)
{
    quaternion q;
...
__device__ __forceinline__ void sortSingularValues(Mat3x3& B, Mat3x3& V)
{
...
__device__ __forceinline__ givens QRGivensQuaternion(float a1, float a2)
{
...
__device__ __forceinline__ QR QRDecomposition(Mat3x3& B)
{
    Mat3x3 Q, R;
...
// in order to avoid certain issues arising from the quaternions becoming too
// small which causes the sqrt to become nan due to it being implemented as a
// reciprocal square root thus causing a division by 0 in the wrong place.
__device__ __forceinline__ SVDSet svd(Mat3x3 A)
{
