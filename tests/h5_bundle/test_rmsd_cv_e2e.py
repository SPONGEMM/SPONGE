...
def producer(request):
    ...
    if not python or not executable:
        pytest.skip(
            f"set {prefix}_PYTHON and SPONGE_EXECUTABLE for real RMSD execution"
        )
...
def _generate(producer, root, rotate):
    ...
    _checked(
        [python, str(script), package, str(root), str(rotate).lower()],
        root,
        env,
    )
...
def _run(producer, case, *, rerun=True, steps=1, restart=None):
    ...
    command = [
        ...
        mdin,
        "-mode",
        "rerun" if rerun else "nve",
        "-step_limit",
        str(steps + 1 if rerun else steps),
        "-dt",
        "0.000001",
        "-print_zeroth_frame",
        "1",
...
def test_rmsd_native_and_converted_legacy_value_and_force(
    producer, tmp_path, rotate
):
    ...
    for name in ("native", "baseline", "legacy"):
        ...
        np.stack([data["positions"]] * 2).astype(np.float32).tofile(
            case / "frames.dat"
        )
        np.savetxt(
            case / "frames.box", [[*np.diag(data["box"]), 90, 90, 90]] * 2
        )
    ...
    for name in ("native", "baseline", "legacy"):
        np.testing.assert_allclose(
            results[name]["rmsd"], [expected] * 2, atol=6e-5, rtol=0
        )
    ...
    np.testing.assert_allclose(
        results["native"]["bias"], [2 * (expected - 0.2) ** 2] * 2, atol=0.006
    )
    expected_force = np.zeros_like(data["positions"])
    expected_force[data["selection"]] = _bias_force(
        selected, data["reference"], rotate
    )
    ...
    np.testing.assert_allclose(
        results["legacy"]["force"],
        results["native"]["force"],
        atol=2e-2,
        rtol=2e-5,
    )
...
def test_rmsd_restart_matches_uninterrupted_nve(producer, tmp_path, rotate):
    ...
    resumed = _run(
        producer, tmp_path / "resumed", rerun=False, steps=2, restart=restart
    )
    np.testing.assert_allclose(
        resumed["rmsd"][-1], continuous["rmsd"][-1], atol=1e-4, rtol=0
    )
    for field in ("position", "force"):
        assert len(resumed[field]) > 0 and len(continuous[field]) > 0
        np.testing.assert_allclose(
            resumed[field][-1], continuous[field][-1], atol=2e-3, rtol=2e-5
        )
    with (
        h5py.File(tmp_path / "continuous/final.spgr.h5") as full,
        h5py.File(tmp_path / "resumed/final.spgr.h5") as split,
    ):
        np.testing.assert_allclose(
            split["/particles/all/position/time"][...],
        ...
        for field in ("position", "velocity"):
            np.testing.assert_allclose(
                split[f"/particles/all/{field}/value"][...],
